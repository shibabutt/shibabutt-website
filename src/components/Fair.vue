<template>
    <section id="fair">
        <div class="container">
            <div class="row header-text text-center" id="features">
                <div class="col-lg-12">
                    <h3>Fair Launch + Community Build</h3>
                </div>
            </div>
            <div class="row over-pa">
                <div class="col-lg-9 col-sm-12 m-auto">
                    <div class="row overview-text">
                        Community driven & fair launch.<br />
                        <u>There is no pre-sale.</u><br />
                        Token will be launched when community reach a consistent number.<br />
                        💰 The team will add 48% of the token supply in the DeFiKingdoms Liquidity Pool.<br />
                        🔥 37% of the supply will be manually burnt at shared milestones (see Tokenomics).<br />
                        ⭐ 15% of the supply will be used for marketing and contests
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>

<script>
    export default {
        name: 'Fair',
        props: {}
    }
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>

</style>
